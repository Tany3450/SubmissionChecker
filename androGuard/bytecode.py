        ...
        for ins in instructions:
            ...
            if ins == instructions[-1] and i.childs:
                ...
                if (op_value == 0x2b or op_value == 0x2c) and len(i.childs) > 1:
                    ...
                    print_fct(' '.join("%d:%s" % (values[j], i.childs[j + 1][2].get_name()) for j in
                                       range(0, len(i.childs) - 1)) + " ]%s" % normal_color)
                else:
                    if len(i.childs) == 2:
...
def method2dot(mx, colors={}):
    ...
    mx.get_method().get_class_name(), mx.get_method().get_name(), mx.get_method().get_descriptor())).hexdigest()
    ...
    for DVMBasicMethodBlock in mx.basic_blocks.gets():
        ...
        for DVMBasicMethodBlockInstruction in DVMBasicMethodBlock.get_instructions():
            ...
            output = ", ".join(
                mx.get_vm().get_operand_html(i, registers, colors, escape, textwrap.wrap) for i in operands)
        ...
        values = None
        if (last_instru.get_op_value() == 0x2b or last_instru.get_op_value() == 0x2c) and len(
                DVMBasicMethodBlock.childs) > 1:
...
def method2json_direct(mx):
    ...
    for DVMBasicMethodBlock in mx.basic_blocks.gets():
        ...
        for DVMBasicMethodBlockInstruction in DVMBasicMethodBlock.get_instructions():
            ...
            if (
                    DVMBasicMethodBlockInstruction.get_op_value() == 0x2b or DVMBasicMethodBlockInstruction.get_op_value() == 0x2c):
